...
<div class="row justify-content-center">
    <div class="col-8">
        <form method="POST" action="{{ route(
            'admin.diagnosis.penyebab.update',
            [
                        ...
                        <div class="col-sm-10">
                            <select class="form-control @error('jenis') is-invalid @enderror" name="jenis">
                                @foreach ($daftar_jenis as $jenis)
                                    <option value="{{$jenis->id_jenis_penyebab}}"
                                        @if ($jenis_penyebab->id_jenis_penyebab == $jenis->id_jenis_penyebab)
                                            selected
                                        @endif
                                    >{{$jenis->nama}}</option>
                                @endforeach
